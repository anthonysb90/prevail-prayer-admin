...
export default function DraftsManager({ rows }: { rows: DraftRow[] }) {
  ...
  const deleteSelected = () =>
    run(() => deleteDevotions(selected), `Permanently delete ${selected.length} draft${selected.length === 1 ? "" : "s"}? This can't be undone.`);
  const deleteEvery = () =>
    ...
    <div className="bg-white rounded-card shadow-card overflow-hidden">
      <div className="flex items-center justify-between gap-3 px-6 py-3 border-b border-line bg-page/50">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button" onClick={deleteSelected} disabled={pending || selected.length === 0}
      ...
      <table className="w-full">
        <tbody className="divide-y divide-line">
          {rows.map((d) => (
            <tr key={d.id} className={`transition-colors ${selected.includes(d.id) ? "bg-brand-soft/40" : "hover:bg-page"}`}>
              <td className="pl-6 pr-2 py-4 w-10">
                <input type="checkbox" checked={selected.includes(d.id)} onChange={() => toggle(d.id)} className="w-4 h-4 accent-brand" />
              </td>
